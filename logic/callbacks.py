...
def register_callbacks(app, df): # 'app' e 'df' são passados aqui
    # TODOS OS CALLBACKS ABAIXO ESTÃO CORRETAMENTE INDENTADOS.
    ...
    @app.callback(
        ...
        Output('page-content-col', 'md'), # Use page-content-col aqui, como definido no layout
        Input('url', 'pathname'),
    ...
    def handle_login(n_clicks, username, password, remember_me_value):
        # print(f"DEBUG: handle_login acionado. N_clicks: {n_clicks}, User: {username}, Pass: {'*' * len(str(password))}, Remember: {remember_me_value}") # Linha de debug opcional

        if not username or not password:
            # print("DEBUG: Campos de usuário/senha vazios.") # Linha de debug opcional
            return no_update, dbc.Alert("Preencha todos os campos.", color="warning", duration=3000)

        user = database.get_user_by_username(username)
        # print(f"DEBUG: database.get_user_by_username retornou: {user.username if user else 'None'}") # Linha de debug opcional
        
        remember = True if 1 in remember_me_value else False # Correção na atribuição de remember

        if user and database.check_password(user.password_hash, password):
            # print(f"DEBUG: Senha correta para {username}. Tentando login_user.") # Linha de debug opcional
            login_user(user, remember=remember)
            # print(f"DEBUG: login_user chamado para {username}. Redirecionando para /.") # Linha de debug opcional
            return "/", "" # Redireciona e limpa mensagem de erro
        else:
            # print(f"DEBUG: Nome de usuário ou senha inválidos para {username}.") # Linha de debug opcional
    ...
    # NOVO CALLBACK: Para alternar a visibilidade do filtro manualmente
    @app.callback(
        Output('filter-toggle-store', 'data'), # Atualiza o estado do filtro no store
        Output('toggle-filter-button', 'children'), # Atualiza o texto/ícone do botão
        Input('toggle-filter-button', 'n_clicks'),
        State('filter-toggle-store', 'data'),
        prevent_initial_call=True
    )
    def toggle_filter_visibility(n_clicks, current_filter_state):
        if n_clicks is None:
            raise exceptions.PreventUpdate

        new_is_hidden = not current_filter_state.get('is_hidden', False)
        new_state = {'is_hidden': new_is_hidden}

        if new_is_hidden:
            button_content = [html.I(className="bi bi-arrows-angle-expand me-1"), " Mostrar Filtro"]
        else:
            button_content = [html.I(className="bi bi-arrows-angle-contract me-1"), " Esconder Filtro"]
